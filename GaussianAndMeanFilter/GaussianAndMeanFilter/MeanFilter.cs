...
namespace GaussianAndMeanFilter
{
    ...
    /// <summary>
    /// Main class for the Mean Filter algorithm using IPipeline
    /// </summary>
    public class MeanFilter : IPipelineModule<double[,,], double[,,]>
    {
        ...
        {
            return MeanConvolutionFilter(data, Mean5x5, 1, 0);
        }
        ...
        /// <returns></returns>
        public double[,,] MeanConvolutionFilter(double[,,] data, double[,] Mean5x5, double factor = 1, int bias = 0)
        {
            ...
            int filterWidth = Mean5x5.GetLength(1);
            int filterHeight = Mean5x5.GetLength(0);
                ...
                for (int offsetX = filterOffset; offsetX < bitmap.Width - filterOffset; offsetX++)
                {
                    ...
                    for (int filterY = -filterOffset; filterY <= filterOffset; filterY++)
                    {
                        for (int filterX = -filterOffset; filterX <= filterOffset; filterX++)
                        {
                            ...
                            blue += (double)(pixelBuffer[calcOffset]) * Mean5x5[filterY + filterOffset, filterX + filterOffset]/25;

                            green += (double)(pixelBuffer[calcOffset + 1]) * Mean5x5[filterY + filterOffset, filterX + filterOffset]/25;

                            red += (double)(pixelBuffer[calcOffset + 2]) * Mean5x5[filterY + filterOffset, filterX + filterOffset]/25;
                        }
                    }
